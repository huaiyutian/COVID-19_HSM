function dur=f8_controlforR0(R0,results2)% ode system function for MCMC algae example
%% load data
...
%calculate R0
beta_b(1:n)=R0./((1-p)*(1./gamma_s+beta_s/sigma_pre)+p*afa./gamma);% basic transmission rate
...
%% model
 for perflow=[0.05:0.05:1] % control intensity of inter-city flow and intra-city flow
    ...
    for perC=[0.05:0.05:1] % control intensity of social distancing 
        ...
        HLa(i,:)=HLa(i-1,:)+p*((afa*beta.*HIa(i-1,:)+beta.*HI(i-1,:)+beta_s*beta.*HLp(i-1,:)).*HS(i-1,:)./HN(i-1,:)+importI)-imcityout_la+imcity_la'-sigma_a*HLa(i-1,:);  
        HIa(i,:)=HIa(i-1,:)+sigma_a*HLa(i-1,:)-gamma.*HIa(i-1,:)-imcityout_ia+imcity_ia';
