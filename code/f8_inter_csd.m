 ...
 for perinter=[0.05:0.05:1] % control intensity of inter-city flow 
    ...
    for perC=[0.05:0.05:1] % control intensity of social distancing
           ...
           %% scene-setting
            ...
            importI=HRwuhan(i-1)*flowWH(i-1,:)*report/Pwuhan*percent_intra(i-1); %; %exposed cases from wuhan to other cities
          ...
          % different class to calculate travel people proportion later 
            ...
            Fcity=Fcitybalcance*percent_intra(i-1);
...
%nationI(find(nationI>365))=370; %duration>1 year set to be 1 class
figure
subplot(2,2,1)
imagesc(nationHI);    %# Create a colored plot of the matrix values
...
set(gca,'XTick',4:5:20,...                         %# Change the axes tick marks
        ...
        'YTickLabel',{'20%','40%','60%','80%','100%'},...
        'TickLength',[0 0],'Fontsize',8);
    title('Average peak size for cities (peak I/N)-log','Fontname', 'Arial','Fontsize',9)
    xlabel('Social distancing,CSD','Fontname', 'Arial','Fontsize',9)
    ylabel('Lockdown,CLD,{minter}','Fontname', 'Arial','Fontsize',9)
    %saveas(gca,'fig4cd.pdf')  