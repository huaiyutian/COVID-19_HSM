...
%         percent_intra(1:simday)=perflow;
%         percent_intra(1:13)=1;%control begin after 1.25 1-level response
    ...
    for perC=[0.05:0.05:1] % control intensity of social distancing
           ...
           %% scene-setting
            ...
            beta2=perC*perintra;%control for transmission between people
            if(i>13)%1.25 1-level response
            beta=beta_b*beta2; %control begin from 1-level response
            end

            % Imported Infected cases from whuhan
            importI=HRwuhan(i-1)*flowWH(i-1,:)*report/Pwuhan; %exposed cases from wuhan to other cities
...
%save matrix
iflow=find(perintra==[0.05:0.05:1] );
...
set(gca,'XTick',4:5:20,...                         %# Change the axes tick marks
    ...
    ylabel('Lockdown,CLD,{mintra}','Fontname', 'Arial','Fontsize',9)
