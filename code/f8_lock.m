...
%HRwuhan=xdata(ceil(t),2*n+2);%wuhan report daily cases--onset
...
[Forder,id]=sort(sum(Fcity,1)+sum(Fcity,2)','descend');%
...
for i = 2:simday
perinner=(innercity(i-1,:)./max_before_fes_inner);
Fcitytem=Fcity;
...
perinnertem(idtem)=0.1;%
beta2=perC*perinnertem; %
...
set(gca,'XTick',4:5:20,...                         %# Change the axes tick marks
    ...
    ylabel('Lock down','Fontname', 'Arial')
