...
HRwuhan(57:simday)=0;
 for perflow=[0.05:0.05:1] %
    for perC=[0.05:0.05:1]%control intensity
for i = 2:simday
beta2=perC*perflow; %
if(i>27)%1.25 lag 2 weeks 2/8
beta=beta_b*beta2; %
end
...
set(gca,'XTick',4:5:20,...                         %# Change the axes tick marks
        ...
        'TickLength',[0 0]);
    title('Length of lockdown for cities£¨day£©')
    xlabel('Social distancing,Csd','Fontname', 'Arial')
    ylabel('Lockdown,CLD','Fontname', 'Arial')
%     
%
sumdur(find(sumdur>365))=370;
figure
...
set(gca,'XTick',4:5:20,...                         %# Change the axes tick marks
        ...
        'YTickLabel',{'20%','40%','60%','80%','100%'},...
        'TickLength',[0 0]);
    title(' duration of nation£¨day£©')
    xlabel('Contact pattern(%)','Fontname', 'Arial')
    ylabel('Movement inflow+inner intensity(%)','Fontname', 'Arial')